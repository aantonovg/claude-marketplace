...
# Set up logging — write to file because LibreOffice GUI redirects stderr to /dev/null
_LOG_PATH = "/tmp/lo_mcp.log"
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)s %(name)s: %(message)s",
    handlers=[logging.FileHandler(_LOG_PATH, mode="a"), logging.StreamHandler()],
)
...
# Process-wide lock so auto-start (background thread on createInstance) and
# the menu-click `trigger("start_mcp_server")` can't both bind the HTTP port.
...
class MCPExtension(unohelper.Base, XJobExecutor, XServiceInfo):
    ...
    def trigger(self, args):
        ...
        try:
            ...
            # The Jobs.xcu auto-start path passes args as a tuple of
            # NamedValue (name='trigger', value='start_mcp_server'). Normalise.
            if isinstance(args, (tuple, list)):
                for a in args:
                    n = getattr(a, "Name", None)
                    v = getattr(a, "Value", None)
                    if n == "trigger" and isinstance(v, str):
                        args = v
                        break
    ...
    def _start_mcp_server(self):
        ...
        try:
            # Hold the lock for the whole start sequence so the second concurrent
            # caller waits, sees `self.started` and bails out.
            with _start_lock:
                if self.started:
                    logger.warning("MCP server is already started")
                    return
                self._do_start_mcp_server()
        except Exception as e:
            logger.error(f"Failed to start MCP server: {e}")
            logger.error(traceback.format_exc())
            self._show_notification("MCP Extension Error", f"Failed to start: {e}")

    def _do_start_mcp_server(self):
        """Actual startup logic, runs under _start_lock."""
        try:
            if self.started:
                return
...
class ExtensionEventListener:
    ...
    def on_extension_load(self, ctx):
        ...
        try:
            ...
            # Auto-start the HTTP server in a background thread so we don't block
            # LibreOffice startup. The race against a possible menu-click
            # `trigger("start_mcp_server")` is resolved by `_start_lock` +
            # `self.started` guard inside `_start_mcp_server`.
            threading.Thread(
